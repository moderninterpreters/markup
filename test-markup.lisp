...
(test write-xml
  (is (equal
       "<foo></foo>"
       (markup:write-xml <:foo></:foo>)))
  (is (equal
       "<foo><bar></bar><car></car></foo>"
       (markup:write-xml <:foo><:bar></:bar><:car></:car></:foo>))))
...
(test write-xml-string
  (is (equal
       "<foo>car dar</foo>"
       (markup:write-xml <:foo>car dar</:foo>))))

(test write-attributes
  (is (equal
       "<foo bar=\"car\"></foo>"
       (markup:write-xml <:foo bar="car"></:foo>))))
...
(test escaping-in-between
  (let ((val "blah"))
    (is (equal
         (markup:write-xml <div>1blah2</div>)
         (markup:write-xml <div>1,(progn val)2</div>)))))

(test escaping-with-space-in-between
  (let ((val "blah"))
    (is (equal
         (markup:write-xml <div>blah car</div>)
         (markup:write-xml <div>,(progn val) ,(progn "car")</div>)))))


(test escaping-attr-vals
  (let ((val "blah"))
    (is (equal
         (markup:write-xml <div arg="blah"></div>)
         (markup:write-xml <div arg=(progn val)></div>)))))
...
(test function-calls
  (is (equal
       (markup:write-xml <:h1>Foo was blah</:h1>)
       (markup:write-xml <wrapper foo="blah"></wrapper>)))
  (is (equal
       (markup:write-xml <:h1>Foo was </:h1>)
...
(test function-calls-2
  (is (equal
       (markup:write-xml <:h1>Foo was blah</:h1>)
       (markup:write-xml <wrapper2 foo="blah"></wrapper2>)))
  (is (equal
       (markup:write-xml <:h1>Foo was defaultvalue</:h1>)
       (markup:write-xml <wrapper2></wrapper2>))))
...
(test function-calls-wrapper
  (is (equal
       (markup:write-xml <:h1>Foo was blah</:h1>)
       (markup:write-xml <wrapper-with-children foo="blah"></wrapper-with-children>)))
  (is (equal
       (markup:write-xml <:h1>Foo was defaultvalue<:blah></:blah></:h1>)
       (markup:write-xml <wrapper-with-children><:blah></:blah></wrapper-with-children>))))
...
(test without-children
  (is (equal "<h1>hello world</h1>"
             (markup:write-xml <without-children />))))
...
(test with-only-key
  (is (equal "<h1>hello zoidberg</h1>"
             (markup:write-xml <with-only-key att="zoidberg" />))))
...
(test escaping
  (let ((body "<body>"))
    (is (equal
         "<h1>&lt;body&gt;</h1>"
         (markup:write-xml <:h1>,(progn body)</:h1>))))
  (let ((body "\"body\""))
    (is (equal
         "<h1 arg=\"&quot;body&quot;\">foo</h1>"
...
(test comments
  (is (equal
       ...
       (markup:write-xml <:body> <!-- this is a test --></:body>))))
